...
def get_etf_registry() -> pd.DataFrame:
    ...
    for url in candidates:
        try:
            ...
            df = df[df["SYMBOL"].str.match(r"^[A-Z]{2,19}$", na=False)]
...
<div class="nse-header">
    ...
    <div class="nse-tagline">
        Market Intelligence &nbsp;&middot;&nbsp;
        <em>{datetime.now(IST).strftime("%d %b %Y")}</em> &nbsp;&middot;&nbsp;
        <em>{datetime.now(IST).strftime("%H:%M IST")}</em> &nbsp;&middot;&nbsp;
...
def scan_etfs(etf_registry: pd.DataFrame, _counter=None, _pbar=None) -> pd.DataFrame:
...
def build_etf_context(etf_df: pd.DataFrame) -> str:
    ...
    for _, r in top.iterrows():
        ...
        lines.append(
            f"{r['ETF']} ({r['Name'][:30]}): Rs.{r['Price']} RSI{r['RSI']:.0f}{rsi_flag} "
...
## Global Macro and Crude Oil

Current Brent crude level, US Fed stance, global risk sentiment.

## India Macro and RBI

Current Nifty and Bank Nifty levels, USD/INR rate, RBI stance, FII/DII flows.

## Key Market Triggers This Week

3-5 bullets. Most important events driving markets.
...
def build_momentum_context(df: pd.DataFrame) -> str:
    ...
    return "\n".join(
        f"{r['Ticker']} Rs.{r['Price']} {r['Signal']} RSI{r['RSI']} "
        f"Vol{r['Vol Ratio']}x Target Rs.{r['Target']}(+{r['Upside %']}%)"
...
def generate_strategy(intel_summary: str, momentum_context: str, sector_context: str, etf_context: str) -> str:
    ...
    system = "NSE equity analyst. Cite both technical AND macro reason per pick. Only use tickers from STOCKS. TOP 3 are not AVOID."
...
with col_note:
    st.markdown(
        ...
        "~2,000 NSE EQ stocks   ETF live data   AI news + strategy   ~5-7 min"
        "</small>",
        unsafe_allow_html=True,
    )
...
if run:
    ...
    st.markdown("<span style='font-family:JetBrains Mono,monospace;font-size:0.7rem;color:#4a5578'>Phase 1 - Market Data</span>", unsafe_allow_html=True)

    _eq_status = st.empty()
    _eq_status.markdown("<span style='font-family:JetBrains Mono,monospace;font-size:0.76rem;color:#8896b3'>Loading equity list ...</span>", unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        f"<span>Equities: <strong style='color:#d4a843'>{n_eq_total:,}</strong></span>"
        f"<span>ETFs: <strong style='color:#00c896'>{n_etfs_clean}</strong></span>"
        f"<span>Signals: <strong style='color:#f0c96a'>{len(momentum_df)}</strong></span>"
        f"</div>",
        unsafe_allow_html=True,
    )

    st.markdown("<span style='font-family:JetBrains Mono,monospace;font-size:0.7rem;color:#4a5578'>Phase 2 - AI Analysis</span>", unsafe_allow_html=True)
    _ai_status = st.empty()
...
if "strategy" in st.session_state:
        ...
        for idx, (_, row) in enumerate(top3.iterrows()):
            ...
            with cols[idx]:
                ...
                <div class="pick-card">
                    ...
                    <div class="ticker-meta">Rs.{row['Price']} &nbsp;RSI {row['RSI']}</div>
                    <span class="upside-pill">+{row['Upside %']}%</span>
                    <div class="target-row">Target &nbsp;<span class="target-price">Rs.{row['Target']}</span></div>
    ...
    st.markdown(
        f"<div class='nse-footer'>Last scan - {st.session_state['last_run'].strftime('%H:%M:%S IST')} - NSE Alpha AI</div>",
        unsafe_allow_html=True,
    )
...
<div class="empty-glyph">NSE</div>
<div class="empty-title">
Hit <strong style="color:#d4a843">Run Market Intelligence</strong> to begin
</div>
<div class="empty-sub">
~2,000 NSE EQ stocks   ETF data   live prices   AI strategy   full momentum table
</div>
</div>
""", unsafe_allow_html=True)